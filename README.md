...
* **训练加载**：每轮以玩家像素为基准随机放大（面积 U(0.7,1)，倍率 1.00–1.195，双线性）、±6px 平移、50% 水平镜像，随后按 106px 圆形重新遮罩。平移小于滑窗步长 8px，Base 中心留在本 tile；镜像恢复八月配方，稳的模型都是翻转不变的。长宽比拉伸关闭；保留颜色扰动。
...
* **背景域泛化**：Base 每个合法中心只保留一次黑底干净锚点，其余出现全部进入图标与模糊增强，背景轮换纹理底与结构化场景（含跨地图干扰碎片）；Tier 恢复八月的黑底、纹理底、结构底 1:1:1，每个分桶都覆盖三种背景，不回滚成全黑底。
